...
exports.verifyCaptcha = async (req, res, next) => {
  try {
    ...
    // 查询数据库内是否存在验证
    const isCodeLegal = await VerificationCode.findOne({
...
exports.getUserProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (bioUri || Object.getOwnPropertyNames(profile).length) {
      console.log("bioUri", bioUri);
      if (bioUri) {
        relativePath = path
          .join("uploads", bioUri.newFileName)
          .replace(/\\/g, "/");
      }
    } else {
      return res.status(401).json({
...
exports.uploadBio = async (req, res, next) => {
  try {
    ...
    const isExsitFile = await FileUploads.findOne({
      where: {
        [Op.and]: [{ uid: req.uid }, { purpose: "bio" }],
      },
    });
    if (isExsitFile) {
      // 如果已存在，更新数据库信息
...
exports.updateUserStatus = async (req, res, next) => {
  try {
    ...
    // 更新数据库
    await User.update(req.body, { where: [{ uid: req.body.uid }] });
    res.status(200).json({
      success: true,
      message: "用户状态更新成功",
    });
